function TimeAPI() {
    var Time = $("#checkedvalue").val();
    var enteredTime = Time;

    var currentTime = new Date();
    var userTime = new Date();
    var hours = enteredTime.split(":")[0];
    var minutes = enteredTime.split(":")[1];
...
function showImagePreviewAPI() { // For API
    let myFile = $('input[id=ImageAPI]').val();
...
function showIconPreviewAPI() { // For API
    let myFile_icon = $('input[id=IconAPI]').val();
    var output_icon = document.getElementById('output11');
    document.getElementById('output11').style.display = "inline-block";
    output_icon.src = myFile_icon;
    return ("Icon Uploaded.")
}

$(function () {
	// Fetching form input fields data.
	$("#web-api").submit(function (event) {
		event.preventDefault();
		var data = new FormData(event.target);
		var arr = [...data.entries()];

		// Adding the firebase cloud messaging device token.
...
$(function () {
	// Fetching form input fields data.
	$("#firebase-api").submit(function (event) {
		event.preventDefault();
		var data = new FormData(event.target);
		var arr = [...data.entries()];
	...
	// Below JS for Notification Click Action URL and Title Fields for API.
	const MAX_SETS = 2;
	...
	const addFieldsBtn = document.getElementById("add-fields-api");

	// Get the input fields div
	const inputFieldsDiv = document.getElementById("input-fields-api");
	...
	if (setIndices.length >= MAX_SETS) {
		alert("Maximum two buttons can be added.");
		return;
	}

	// Get the next set index
	const setIndex = setIndices.length;
	...
	deleteBtn.addEventListener("click", () => {
		...
		const index = setIndices.indexOf(setIndex);
		setIndices.splice(index, 1);

		// Remove the set div from the input fields div
		inputFieldsDiv.removeChild(setDiv);
